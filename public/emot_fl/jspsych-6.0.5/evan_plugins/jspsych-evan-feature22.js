...
jsPsych.plugins["evan-feature22"] = (function() {
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      neg_image: { // image negative distractor
        type: jsPsych.plugins.parameterType.IMAGE,
        default: undefined
      },
      pos_image: { // image positive distractor
        type: jsPsych.plugins.parameterType.IMAGE,
        default: undefined
      },
      neutral_image: { // image neutral distractor
        type: jsPsych.plugins.parameterType.IMAGE,
        default: undefined
      },
      c1_feature_probs: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    //emotion distraction stimuli
    var emotion_time = 500;
    ...
    var emotion_images = [trial.emot_image];
    ...
    var emotion_img_width= w/2;
    var emotion_img_height = h/2;
    ...
    var choice_image_x_arr = [2*w/5 - choice_img_width/2,
                              3*w/5 - choice_img_width/2];

    var emotion_image_y = h/2 - emotion_img_height/2;
    var emotion_image_x = w/2 - emotion_img_width/2;
    // goes to h/5
    var choice_y_stage2 = h/5 - choice_img_height/2;

    // y position of features
    var f_rad = choice_img_width/4;
    var f_y_arr = [2*h/5, 3.5*h/5];
    var final_text_y = 2.75*h/5;
    ...
    var display_choice_stim_wait_for_response = function(){
      ...
      var display_chosen_state_features = function(){
        ...
        var emotionscreen = d3.select("svg").append("svg:image").attr("class", "emotion img").attr("x", emotion_image_x)
            .attr("y", emotion_image_y).attr("width",emotion_img_width).attr("height",emotion_img_height)
            .attr("xlink:href", emotion_images[0]).style("opacity",1);
        jsPsych.pluginAPI.setTimeout(function() {
        emotionscreen.remove();
        }, emotion_time);
        jsPsych.pluginAPI.setTimeout(function() { // wait for time after
          ...
          for (f_idx = 0; f_idx < 2; f_idx++){
            ...
            if (!trial.single_choice_option){
              jsPsych.pluginAPI.setTimeout(function() {
                for (f_idx = 0; f_idx < 2; f_idx++){
                  if (chosen_state_features[f_idx]){
                    d3.select("svg").append("text")
                                .attr("class", "reward")
                                .attr("x", f_x + 1.5*f_rad)
                                .attr("y", f_y_arr[f_idx] + h/60) //+ 1.25*f_rad)
                                .attr("font-family","Helvetica")
                                .attr("font-weight","light")
                                .attr("font-size",h/30)
                                .attr("text-anchor","middle")
                ...
                if (reward_received > 0){
                  var this_text = `Won ${reward_received}`;
                  var this_color = win_color;
                }else if(reward_received < 0){
                  var this_text = `Lost ${reward_received}`;
                ...
                jsPsych.pluginAPI.setTimeout(function() {
                    if (!trial.single_choice_option){
                      d3.select("svg").append("text")
                                  .attr("class", "reward")
                                  .attr("x", f_x)
                                  .attr("y", final_text_y)//3.65*h/5)//f_y_arr[1] + h/120 + 3.5*f_rad)
                                  .attr("font-family","Helvetica")
                                  .attr("font-weight","light")
                                  .attr("font-size",h/25)
                                  .attr("text-anchor","middle")
             ...
             // how long to wait for emotion distraction
      }
